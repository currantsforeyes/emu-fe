...
// ============================================================================
// IPC HANDLERS - Scraping Operations
// ============================================================================

// Scan ROM directory for files
ipcMain.handle('scraper:scan-rom-directory', async (event, dirPath: string, extensions: string[]) => {
  try {
    if (!fs.existsSync(dirPath)) {
    ...
    console.error('Error scanning ROM directory:', error);
    return { success: false, error: error.message, files: [] };
  }
});

// Download asset from URL to local path
ipcMain.handle('scraper:download-asset', async (event, url: string, destPath: string) => {
  try {
...
// Get assets directory for a system
ipcMain.handle('scraper:get-assets-directory', async (event, systemId: string) => {
  try {
    const assetsDir = path.join(app.getPath('userData'), 'assets', systemId);
    ...
    console.error('Error getting assets directory:', error);
    return { success: false, error: error.message };
  }
});

// Load game database from file
ipcMain.handle('scraper:load-game-database', async () => {
  try {
    const dbPath = path.join(app.getPath('userData'), 'games-database.json');
    
    if (!fs.existsSync(dbPath)) {
      return { success: true, data: [] }; // Empty database
    }
...
ipcMain.handle('scraper:save-game-database', async (event, games: any[]) => {
  try {
    const dbPath = path.join(app.getPath('userData'), 'games-database.json');
    fs.writeFileSync(dbPath, JSON.stringify(games, null, 2), 'utf-8');
